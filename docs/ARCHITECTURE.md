...
> Photoshop step. What REMAINS from today: Ghostscript removed entirely (no
> shell-out, no `/api/export` endpoint; exports download straight from the browser),
...
Style on disk (styles/<NAME>/)          Customer artwork PDF        Quantity
  ...
  VERIFY GATE (src/lib/verifyArtwork.js) — blocks export on missing/mismatched
        ...
        ▼
  EXPORT
   • Direct vector (RasterLink-proven) — one button, downloaded straight
     from the browser (PDF + laser DXF); the app never flattens
        │
        ▼
...
7. **Inputs keep vectors intact — and the app does not flatten, period.** The
   export is always the direct-vector composition. Flattening heavy layered
   artwork is a customer-side Photoshop step (Layer → Flatten Image), done with
   color authority before upload — the app only detects and warns (the in-app
   flatten tried on 2026-07-14 made files ~6× larger and was reverted same day).
...
9. **Direct-vector export is the ONLY path.** The old Ghostscript flatten fallback
   was removed entirely (owner sign-off, 2026-07-14) — no shell-out, no
   `/api/export` endpoint — and its short-lived in-app replacement was reverted the
   same day (see §7). There is no flatten mechanism in the app at all; the export
   downloads straight from the browser.
